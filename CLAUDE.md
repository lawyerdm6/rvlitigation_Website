...
## Current State (July 29, 2026 — sixth session, commit `99e9cb9`, live on origin/main)
- **Homepage practice-area cards: added a third card, "Private Civil Matters"**, linking to `civil-litigation/civil-lawsuit-defense`. Uses `images/Icons/CIvil_icon.png` — note the user replaced this icon's *contents* via GitHub web upload mid-session (commit `244b2b8`, same out-of-band-upload pattern as the Chelsie/Cristy photos earlier — pulled in via `git pull --rebase` before making local edits). The three cards still use the same `width: calc((100% - 30px) / 2); max-width: 260px;` flex-wrap layout as before; the third card just wraps to its own centered row rather than getting a new 3-column layout — not asked to change that, left as-is.
- **`civil-litigation/civil-lawsuit-defense.html`**: added a new "Types of Private Civil Matters We Handle" section — a 20-item `<ul class="plain-list">` (same list style already used higher up in the page's "How We Assist" section, not card/box styling) — positioned directly below the "How We Help" (`#how-we-help`) section and before FAQ, per explicit placement instruction. Added to the sticky TOC as well. Content is intentionally broad (contract, guarantee, fraud, HOA, construction defect, defamation, fiduciary duty, partnership dissolution, debt collection, civil harassment, probate/trust contest, quiet title, easement, judgment enforcement, nuisance, vehicle/property damage, wrongful eviction, elder financial abuse, small claims appeal) for SEO breadth, while staying consistent with the page's existing "no contingency, no civil-rights" positioning — nothing on the list falls outside that scope.

## Prior State (July 29, 2026 — fifth session, commit `d160d7c`, live on origin/main)
...
- **New "Stages of a [Civil Lawsuit / Criminal Case] in California" section** added below the two-column block on each page — 6 numbered steps, reusing the existing `.values-list`/`.value-item`/`.value-number` component (already used by "Why Choose Us" further down the same pages) rather than inventing a new pattern.
  - Civil: Filing the Complaint → Defendant's Response (CCP 412.20, 30 days) → Discovery → Pre-Trial Motions & Settlement → Trial (preponderance of the evidence) → Post-Trial & Appeal.
  - Criminal: Arrest & Booking → Arraignment → Pre-Trial Proceedings (PC 872 preliminary hearing, PC 1538.5 suppression motions) → Plea Negotiations → Trial (beyond a reasonable doubt) → Sentencing & Appeal.
- **New "Key Takeaways" section** added below the stages on each page — 4-card grid, new `.takeaways-grid`/`.takeaway-card` CSS added to `css/styles.css` (didn't exist before; hub pages had no comparable "highlight card grid" component, only the numbered `.value-item` list). Covers, per the user's explicit list: strict deadlines (CCP 337/338/339 for civil statutes of limitations; PC 1382 speedy-trial for criminal), settlement/plea negotiation, negotiation happening throughout a case, and litigation-readiness-as-leverage.
- Validated: HTML tag-balance parser clean on both files, CSS brace count balanced, both pages return HTTP 200 locally, all 6 stage headings + 4 takeaway cards confirmed present per page via curl+grep.

## Prior State (July 28-29, 2026 — fourth session, pushed)
